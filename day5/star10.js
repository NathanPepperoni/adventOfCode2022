...
function executeMove(move, stack) {
    stack[move.dest] = stack[move.dest].concat(stack[move.target].slice(-move.count));
    stack[move.target] = stack[move.target].slice(0, stack[move.target].length - move.count);
}
